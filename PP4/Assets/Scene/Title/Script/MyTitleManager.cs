...
public class MyTitleManager : MonoBehaviour
{
	...
	/// </summary>
	[SerializeField]
	float m_messagAColor;

	/// <summary>
	/// スタートメッセージのテキストのa値のプラスマイナス切り替え
	/// </summary>
	bool m_messagAColorPlus;

	/// <summary>
	/// スタートメッセージのテキストのa値のプラスマイナス切り替え
	/// </summary>
	[SerializeField]
	float m_changeColorSpeed;
	...
	{
		if (m_messagAColor > 0 && !m_messagAColorPlus)
		{
			m_messagAColor -= m_changeColorSpeed;
		}
		else if (m_messagAColor < 1 && m_messagAColorPlus)
		{
			m_messagAColor += m_changeColorSpeed;
		}
		...
		if (m_messagAColor >= 1)
		{
			m_messagAColorPlus = false;
		}
		else if (m_messagAColor <= 0)
		{
			m_messagAColorPlus = true;
		}
		StartMessage.GetComponent<Text>().color =
			new Color(StartMessage.color.r, StartMessage.color.g, StartMessage.color.b, m_messagAColor);
	}
}
